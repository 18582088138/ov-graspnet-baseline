...
from pointnet2.pointnet2_utils import  ball_query, QueryAndGroup

model_name = 'BallQuery'
device = "CPU"
...
class SelfModel(nn.Module):
    ...
    def forward(self, radius, nsample, xyz, new_xyz):
        idx_output = ball_query(radius, nsample, xyz, new_xyz)
...
torch_cpu_output = torch_model(radius, nsample, xyz, new_xyz)
...
onnx_input = (radius, nsample, xyz, new_xyz)
onnx_input_name = ['radius', 'nsample', 'xyz', 'new_xyz']
...
torch_gpu_output = torch_gpu_model(radius.to(gpu_device), nsample.to(gpu_device), xyz.to(gpu_device), new_xyz.to(gpu_device))
